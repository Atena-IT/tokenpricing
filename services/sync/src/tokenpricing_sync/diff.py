...
def compare_datasets(
    previous: PricingData | None, current: PricingData
) -> dict[str, Any]:
    if previous is None:
        return {
    ...
    for model_id, current_model in current_models.items():
        ...
        if (
            current_model.pricing.input_per_million
            != previous_model.pricing.input_per_million
            or current_model.pricing.output_per_million
            != previous_model.pricing.output_per_million
        ):
            changes.append(
                {
        ...
        if (
            current_model.pricing.cache_read_per_million
            != previous_model.pricing.cache_read_per_million
            or current_model.pricing.cache_creation_per_million
            != previous_model.pricing.cache_creation_per_million
        ):
            changes.append(
                {
    ...
    return {
        ...
        "summary": {
            "model_additions": sum(
                1 for change in changes if change["type"] == "model_added"
            ),
            "model_removals": sum(
                1 for change in changes if change["type"] == "model_removed"
            ),
            "pricing_changes": sum(
                1 for change in changes if change["type"] == "pricing_changed"
            ),
            "cache_price_changes": sum(
                1 for change in changes if change["type"] == "cache_price_changed"
            ),
        },
        "changes": changes[:500],
    }
